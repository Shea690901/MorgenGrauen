...
static int _verfolge(string str)
{
  ...
  if (!strlen(str))
  {
    ...
    if (pur[0]) pur[0]->RemovePursuer(this_object());
    printf("Verfolgungsmodus abgeschaltet.\n");
    return 1;
  }
  str=lower_case(str);
  if (intp(lv=match_living(str)))
  {
    if (lv==-2)
      write("Kein solches Wesen gefunden.\n");
    else
      printf("verfolge: '%s' ist nicht eindeutig.\n",capitalize(str));
    return 1;
  }
  if (!(lv2=find_player(lv))) lv2=find_living(lv);
  if (!lv2)
  {
    printf("verfolge: Unerklaerlicher Fehler beim Finden von '%s'.\n",
           capitalize(lv));
    return 1;
  }
  lv2->AddPursuer(this_object());
  printf("Du verfolgst jetzt %s.\n",lv2->name(WEN));
  lv2->TakeFollowers();
  return 1;
}
