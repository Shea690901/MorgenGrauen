...
private int act_attr_heal_poison(int effekt)
{
  ...
  if (prot_poison > 0)
  {
    ...
    int *res=({});
    // H_HOOK_POISON haben wir vermutlich noch nicht registriert, aber
    // H_HOOK_INSERT koennten wir schon haben, daher nur versuchen, wenn wir
    // noch keine Konsumenten sind.
    if (!environment()->HIsHookConsumer(H_HOOK_POISON, #'hcallback))
      res += ({ reg_hook(H_HOOK_POISON, H_DATA_MODIFICATOR) });
    if (!environment()->HIsHookConsumer(H_HOOK_INSERT, #'hcallback))
...
private int act_attr_heal_disease(int effekt)
{
  ...
  if (prot_disease > 0)
  {
    ...
    foreach(object o: ob)
    {
      ...
      if (disease <= prot_disease*10)
      {
        prot_disease -= disease/10 + 1;
        o->SetProp(P_LEVEL, 0);
        if (o->remove())
          ++erfolgreich;
      }
      ...
      {
        disease -= prot_disease * 10;
        prot_disease = 0;
        o->SetProp(P_LEVEL, disease);
        if (environment()->QueryProp(P_POISON) == disease)
          ++erfolgreich;
      }
      if (prot_disease <= 0)
        break;
...
private int act_attr_change_dimension(int effekt)
{
  ...
  // nur -600 < effekt < 0 froscht instantan ;-)
  if (effekt < -600)
  {
    // Verzoegerung ausrechnen und dies hier per call_out nochmal mit einem
    // effekt rufen, der eine instantane Reaktion ausloest.
