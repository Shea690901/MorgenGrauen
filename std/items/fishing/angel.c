...
static int angel(string str) {
  ...
  if ( environment(ME) != TP ) {
    ...
    tell_object(TP, "Ohne Koeder am Haken wird sich kein Fisch der Welt "
      "fuer Deine Angel interessieren.\n");
  }
...
static void do_angel() {
            ...
            active != room->QueryProp(P_WATER) )
  {
    ...
    msg_self = fisch->Name(WER)+" biss an, war aber zu schwer fuer "
      "Dich und Deine Angel. Er entgleitet Dir und plumpst zurueck ins "
      "Wasser, weil du zu voll beladen bist!";
