...
protected int CheckTelnetKeepAlive() {
  if (telnet_tm_counter > 0) {
    ...
    if (!(--telnet_tm_counter)) {
      ...
      // alle 300 HBs (600s, 10min).
      telnet_tm_counter = 300;
    }
...
void save_me(mixed value_items)
{
  ...
  autoload=([]);
  foreach(object ob: deep_inventory(ME)) {
    if( val = ob->QueryProp( P_AUTOLOADOBJ ) )
    {
  ...
  dummy=GUILD_DIR+QueryProp(P_GUILD);
  if (find_object(dummy) || file_size(dummy+".c")>-1)
