...
void doWieldMessage()
{
  ...
  if(QueryProp(P_WIELD_MSG))  // Ist eine WieldMsg gesetzt?
  {
    if(closurep(QueryProp(P_WIELD_MSG)))  // Sogar als eigene Fkt.?
    {
      ...
      {
        write(break_string(str[0],78, 0, BS_LEAVE_MY_LFS));
      }
      ...
      //Daher nun Ausgabe an das Env vom Env (es ist fuer tell_room() egal, 
      //ob das nen Raum ist oder nicht, daher keine Pruefung, ob es wirklich
      //ein Raum ist. ;-)
      //say(break_string(str[1],78, 0, BS_LEAVE_MY_LFS),({ this_player() }));
      if ( objectp(environment()) && objectp(environment(environment())) )
          tell_room(environment(environment()),
	    break_string(str[1],78, 0, BS_LEAVE_MY_LFS),({ this_player()}) );
    ...
    else if(interactive(this_player()))
    {
      ...
      write(break_string(s1,78, 0, BS_LEAVE_MY_LFS));
    }
    ...
    //say(break_string(s1,78, 0, BS_LEAVE_MY_LFS ),({ this_player() }));
    if ( objectp(environment()) && objectp(environment(environment())) )
	tell_room(environment(environment()),
	  break_string(s1,78, 0, BS_LEAVE_MY_LFS ),({this_player()}) );
  ...
  {
    write(break_string("Du zueckst "+name(WEN,1)+".",78));
  }
...
void doUnwieldMessage(object wielded_by)
{
  ...
  if(QueryProp(P_UNWIELD_MSG))  // Hier gibt es eine UnwieldMsg.
  {
    if(closurep(QueryProp(P_UNWIELD_MSG)))  // Sogar als Closure, wow :-)
    {
      ...
      {
        tell_object(wielded_by,break_string(str[0],78, 0, BS_LEAVE_MY_LFS));
      }
      //(Zesstra): say() ist hier bloed, weil es an das Env vom this_player()
      //ausgibt, sofern der existiert. So koennen Spieler die meldung kriegen,
      //obwohl die laengst woanders sind (z.B. Sequenzen)
      ...
      //ob das nen Raum ist oder nicht, daher keine Pruefung, ob es wirklich
      //ein Raum ist. ;-)
      //say(break_string(str[1],78,0, BS_LEAVE_MY_LFS),({ wielded_by }));
      if ( objectp(environment()) && objectp(environment(environment())) )
	  tell_room(environment(environment()),
	      break_string(str[1],78,0, BS_LEAVE_MY_LFS),({ wielded_by }) );
    ...
    else if(interactive(wielded_by))
    {
      ...
      tell_object(wielded_by,break_string(s1,78, 0 , BS_LEAVE_MY_LFS));
    }

    s1 = replace_personal(sprintf(QueryProp(P_UNWIELD_MSG)[1],"@WEN1","@WEN2"),
		    ({wielded_by, this_object()}), 1);
    
    //s.o., ersetzt durch tell_toom()
    //say(break_string(s1,78, 0, BS_LEAVE_MY_LFS ),({ wielded_by }));
    if ( objectp(environment()) && objectp(environment(environment())) )
	tell_room(environment(environment()),
	    break_string(s1,78, 0, BS_LEAVE_MY_LFS ),({ wielded_by }) );
  ...
  {
    tell_object(wielded_by,break_string("Du steckst "
     +name(WEN,1)+" zurueck.",78));
  }
  //s.o., ersetzt durch tell_room()
  //say(break_string(wielded_by->Name(WER)+" steckt "
  // +name(WEN,0)+" zurueck.",78),({ wielded_by }));
  if ( objectp(environment()) && objectp(environment(environment())) )
      tell_room(environment(environment()),break_string(wielded_by->Name(WER)
	    +" steckt "+name(WEN,0)+" zurueck.",78),
	  ({wielded_by }) );
}
