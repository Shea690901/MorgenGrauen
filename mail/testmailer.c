...
#undef MAILDEMON0297    // Maildemon-Version 8.Feb.97 vorhanden?
...
reset() {
  object pl;
  (::reset());
  if (!name) return remove();
  pl=find_player(name);
  if (!pl || environment()!=pl) return remove();
...
static varargs write_mail(mixed str, std_subject, text) {
  ...
  if (((catch(h=file_size("/"+SAVEPATH+str2[0..0]+"/"+str2+".o"))),h<=0 
       && member(str2,'@')==-1))
    return write("Mit dem Namen gibt es hier niemanden.\n"),(directflag?remove():input());
  receiver=str;
...
static get_carbon_copy(str) {	// Aufruf mit 0, wenn keine cc gewuenscht.
  int i,j;
  object p;
...
/*  if (!sizeof(carbon)) carbon=0; */
  ...
  orignames=([]);
  h=old_explode(FWSERV->QueryForward(receiver),",");
  ...
  for (i=sizeof(carbon)-1;i>=0;i--) {
    h=old_explode(FWSERV->QueryForward(carbon[i]),",");
    for (j=sizeof(h)-1;j>=0;j--) {
      h[j]=(h[j][0]=='\\'?h[j][1..]:h[j]);
  ...
  if (!pointerp(carbon) || !sizeof(carbon)){
    ...
    for (i=0;i<sizeof(carbon);i++){
      ...
      for (j=sizeof(a=orignames[carbon[i]]-({carbon[i]}))-1;j>=0;j--) 
...
-----------------------------\n";
  if (!appendflag) return get_carbon_copy(),1;
  else {
...
static mail_cmds(str) {
  ...
  case "g":
    if (nrargs<2) { 
      if (Reply(akt_nr-1,(strargs[0][0]=='g'))) return 1;
      break;
    }
    ...
    if (Reply(to_int(strargs[1])-1,(strargs[0][0]=='g'))) return 1;
    break;
  case "weit":
  case "f":
    if (nrargs<2 || 
  ...
  case "F":
    ...
    if (Forward(strargs[2..],to_int(strargs[1])-1,1)) return 1;
    break;
  case "lies":
    if (nrargs<2) { if (ReadMessage(akt_nr-1)) return 1; } else
    if (ReadMessage(to_int(strargs[1])-1)) return 1;
    break;
  case ".":
  ...
  case "-":
    ...
    else if (ReadMessage(akt_nr-2)) return 1;
    break;
  case "spei":
  case "s":
...
static mixed process_names(mixed s) {
  ...
  int i;
  string domain;
  if (stringp(s)) a1=explode(lower_case(s)," ");
  else a1=s;
  a2=({});
  for (i=0;i<sizeof(a1);i++) a2+=explode(a1[i],",");
  a1=({});
