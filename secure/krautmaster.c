...
mapping make_potion(object* plants)
{
  ...
  foreach (object pl:plants)
    {
      //debug(sprintf("Gehe durch Plant: %O",pl));
      int id=pl->QueryPlantId();
      string key=id2key[id];
      // Zu den Wirkungen noch den Key hinzufuegen. Jaja. Besser.
      // Brauch ich noch. Und die Qualitaet auch
      //debug(key);
      //debug(sprintf("%O",ingredients));
