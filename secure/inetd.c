...
/* #define UDP_MASTER		"/secure/master" */
/* #define UDP_MASTER		"obj/master" */

/* The function that is called at load-time if not create() or reset(). */
/* #define CREATE_FUN()		teleledningsanka()	// CD2 */
/* #define CREATE_FUN()		>>()			// CD3 */
...
/* #define deep_copy(x)	copy(x)			// MudOS */
/* #define deep_copy(x)	(x + ([ ]))		// CD */
/* #define call_out(fun, del, arg) \
		set_alarm(itof(del), 0.0, fun, arg)		// CD3 */
/* #define send_udp(host, port, data) \
...
/* Define if your system does not support closures. (CD / MudOS) */
#undef NO_CLOSURES
...
void set_host_list() {
	...
	old_hosts = hosts;
	hosts = ([ ]);
	for(i = sizeof(data = old_explode(data, "\n")); i--; ) {
	    if (data[i] == "" || data[i][0] == '#')
		continue;
	    if (sizeof(data[i] = old_explode(data[i], HOSTFILE_DEL)) < 5) {
	    ...
	    if (member_array("*",
	    local_cmds = old_explode(data[i][LOCAL_COMMANDS], HOSTFILE_DEL2)) != -1)
		...
		local_cmds,
		old_explode(data[i][HOST_COMMANDS], HOSTFILE_DEL2),
		UNKNOWN
	    });
	    /*
