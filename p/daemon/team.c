...
private static mapping is_member;   // Teammitglieder
private static object leader;       // Teamleiter
private static string tname;        // Teamname
private static mapping wanted_row;  // Gewuenschte Reihe
private static mapping wimpy_row;   // Fluchtreihe
private static mapping act_row;     // Aktuelle Reihe
private static mapping autofollow;  // Spieler folgt Teamleiter
private static mapping attack_cmd;  // Spieler hat Angriffsbefehl
private static mapping assoc_mem;   // Zugeordnete Mitglieder (Kampf NPCs)
private static int *formin,*formax; // Formation
private static mixed *rows;         // Die Reihen
private static int last_reorder;    // Letzte Formationspruefung
private static mapping h;           // Sortier-Score: 125*G.Reihe-HP
private static object *att_exec;    // Mitglieder, die Attacke ausfuehren.
private static object *mis_attacked;// (Ex-)Mitglieder ohne Begruessungsschlag
private static mapping mis_init_att;// Fehlende Begruessungsschlaege
private static mapping hp_info;     // HP, MAX_HP, SP, MAX_SP
private static int autoinf_flags;
private static mapping autoinf_hp;
private static mapping autoinf_sp;
private static int autoinf_time;
...
void create() {
  ...
  hp_info=([]);
  hist=({});
  if (object_name(ME)=="/p/daemon/team")
    return;
...
varargs private static void gtell(string str, string who, int tohist) {
  int i;
...
varargs private static int *GetHpInfo(object ob, closure cl) {
  int *res;
...
private static void UpdateActRow() {
  int i,j,update_hp;
  object *old,*new,ob;
  mixed aso;
  ...
  for (i=MAX_TEAMROWS-1;i>=0;i--) {
    old=rows[i];new=({});
    for (j=sizeof(old)-1;j>=0;j--) {
      if (objectp(ob=old[j]) && is_member[ob] && !act_row[ob]) {
        act_row[ob]=i+1;
        new+=({ob});
        if (update_hp) {
...
private static void MakeFormation() {
  // Verlegungsstrategie:
...
private static void RemoveFromRow(object ob, int src) {
...
private static void AddToRow(object ob, int dest) {
...
private static void CycleRows(object ob, int src, int dest) {
  int i;
...
private static void RandomChangeRow() {
  int *nums,i;
  object p1,p2;
...
static void DoChangeRow(object pl, int dest) {
  ...
  dest--;
  CheckFormation();
  h=0;UpdateActRow(); // h=0 damit HP-Liste geupdated wird.
  ...
  efun::m_delete(ecb,find_object("/room/netztot")); // Das gaebe Mega-Lag :-)
...
private static int ChangeRow(string arg) {
  int num;
...
private static int ChangeWimpyRow(string arg) {
  int num;
...
mixed *PresentRows(object env) {
  ...
  for (i=0;i<MAX_TEAMROWS;i++) {
    old=rows[i];new=({});
    for (j=sizeof(old)-1;j>=0;j--) {
      if (objectp(ob=old[j]) && is_member[ob] && environment(ob)==env) {
...
private static void DoChangeLeader(object ob) {
...
private static int ChangeLeader(string arg) {
  object ob;
...
varargs private static int CheckSecond(object pl, object *obs) {
  mixed x,ip;
  mapping second;
  object ob;
  int i;
...
private static void DoAddMember(object ob) {
  closure cl;
...
int AddMember(object ob) {
  if (!Allowed(TP) || !Allowed(ob))
    return 0;
  if (TP!=ob && CheckSecond(ob,({TP}))) // Zweitiereglung bei Gruendung.
    return notify_fail(ob->Name(WER)+" ist Zweitspieler von Dir.\n"),0;
  if (!IsMember(leader))
    AddMemberAndAssocs(TP);
  if (TP!=leader)
    return notify_fail("Nur der Teamleiter kann Mitglieder aufnehmen.\n"),0;
...
private static void DoRemoveMember(object ob) {
  object *tmembers,caster;
  mixed asmem;
  int i;

  ob->SetProp(P_TEAM,0);
  efun::m_delete(is_member,ob);
  efun::m_delete(wanted_row,ob);
  efun::m_delete(act_row,ob);
  efun::m_delete(wimpy_row,ob);
  efun::m_delete(autofollow,ob);
  efun::m_delete(attack_cmd,ob);
  ...
  efun::m_delete(hp_info,ob);
  efun::m_delete(autoinf_hp,ob);
  efun::m_delete(autoinf_sp,ob);
  DoChangeRow(ob,-1);

  if (!objectp(assoc_mem[ob])) {
    if (ob->QueryProp(P_GHOST)) {
      gtell(CAPITALIZE(ob->name(WER))+" HAT DAS TEAM VERLASSEN.","Tod");
    } else {
      tell_object(ob,"Du verlaesst das Team.\n");
      gtell(ob->Name(WER)+" hat das Team verlassen.\n");
    }
  }
  efun::m_delete(assoc_mem,ob);
...
private static void RemoveSingles() {
  object *obs;
  mixed aso;
...
private static int StartAttack() {
...
private static void DoNotifyHpChange() {
  object *obs,ob;
  string str;
  int i;
