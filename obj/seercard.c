...
static string _guthaben()
{
    ...
    return "Dein Guthaben betraegt " + ((i > 1) ? to_string(i) : "eine") +
        " Muenze" + ((i > 1) ? "n" : "") + ".\n";
}
...
static mixed _query_autoloadobj()
{
    ...
    // einem Paket o.ae. steckt (zweiter Teil der Abfrage) bzw. wenn sie
    // in einem pseudo-leblosen Objekt steckt (renew - dritte Abfrage)
    if ( BESITZER_GUELTIG ||
         environment() && environment(environment()) &&
         query_once_interactive(environment(environment())) &&
         getuid(environment(environment())) == owner ||
         previous_object(1) &&
         object_name(previous_object(1)) == "/secure/master" )
        return owner;
...
static mixed _set_autoloadobj( mixed wert )
{
    // Wird normalerweise nur beim Einloggen nach Ende/Reboot aufgerufen
    if ( owner == "" && stringp(wert) && this_interactive() &&
         getuid(this_interactive()) == wert ){
...
static int _abheben( string str )
{
    ...
    this_player()->Set( KONTO, guthaben - summe );
...
// Die eigentlich wichtige Funktion: das Zusammenfassen mit Muenzen im Inv.
// Da die Seer-Card eine Guthabenkarte ist, wird nur mit negativen Muenzen
// zusammengefasst. Und natuerlich nur, wenn es die eigene Karte ist. ;-)
public int IsEqual( object ob )
{
    int muenzen, guthaben;
    
    if ( BLUE_NAME(ob) != "/obj/money" ||
         (muenzen = (int) ob->QueryProp(P_AMOUNT)) >= 0 ||
         !(guthaben = _query_amount()) )
        return 0;

    // negativen(!) Betrag abziehen
    guthaben += muenzen;
    
    if ( guthaben < 0 ){
        // Karte leer - Restbetrag wieder zurueck
        ob->AddAmount( guthaben - muenzen );
        environment()->Set( KONTO, 0 );
    }
    else {
        ob->AddAmount( -muenzen );
        environment()->Set( KONTO, guthaben );
    }
}
